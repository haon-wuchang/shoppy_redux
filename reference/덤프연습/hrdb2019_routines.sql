...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_getproduct` AS select `shoppy_product`.`PID` AS `pid`,`shoppy_product`.`PNAME` AS `name`,`shoppy_product`.`PRICE` AS `price`,`shoppy_product`.`DESCRIPTION` AS `info`,`shoppy_product`.`UPLOAD_FILE` AS `uploadFile`,`shoppy_product`.`SOURCE_FILE` AS `sourceFile`,`shoppy_product`.`PDATE` AS `pdate`,concat('http://localhost:9000/',json_unquote(json_extract(`shoppy_product`.`UPLOAD_FILE`,'$[0]'))) AS `image`,json_array(concat('http://localhost:9000/',json_unquote(json_extract(`shoppy_product`.`UPLOAD_FILE`,'$[0]'))),concat('http://localhost:9000/',json_unquote(json_extract(`shoppy_product`.`UPLOAD_FILE`,'$[1]'))),concat('http://localhost:9000/',json_unquote(json_extract(`shoppy_product`.`UPLOAD_FILE`,'$[2]')))) AS `imgList`,json_arrayagg(concat('http://localhost:9000/',`jt`.`filename`)) AS `detailImgList` from (`shoppy_product` join json_table(`shoppy_product`.`UPLOAD_FILE`, '$[*]' columns (`filename` varchar(100) character set utf8mb4 path '$')) `jt`) group by `shoppy_product`.`PID` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_orderlist` AS select `sm`.`ID` AS `id`,`sm`.`NAME` AS `name`,`sm`.`PHONE` AS `phone`,concat(`sm`.`EMAILNAME`,'@',`sm`.`EMAILDOMAIN`) AS `email`,`sm`.`ZIPCODE` AS `zipcode`,`sm`.`ADDRESS` AS `address`,`sp`.`PID` AS `pid`,`sp`.`PNAME` AS `pname`,`sp`.`PRICE` AS `price`,`sp`.`DESCRIPTION` AS `info`,concat('http://localhost:9000/',json_unquote(json_extract(`sp`.`UPLOAD_FILE`,'$[0]'))) AS `image`,`sc`.`CID` AS `cid`,`sc`.`SIZE` AS `size`,`sc`.`QTY` AS `qty` from ((`shoppy_member` `sm` join `shoppy_cart` `sc`) join `shoppy_product` `sp`) where ((`sm`.`ID` = `sc`.`ID`) and (`sc`.`PID` = `sp`.`PID`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
